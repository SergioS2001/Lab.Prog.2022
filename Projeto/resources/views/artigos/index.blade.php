@extends('layouts.layout')

@section('content')
<div class="row">
    <div class="col-lg-12 margin-tb">
        <div class="pull-left">
            <h2>Tesste para criar novoss artigos </h2>
        </div>
        <div class="pull-right">
            <a class="btn btn-success"  href="{{ route('artigos.create') }}"> Criar Artigo</a>
        </div>
    </div>
</div>

@if ($message = Session::get('success'))
<div class="alert alert-success">
    <p>{{ $message }}</p>
</div>
@endif

<table class="table table-bordered">
    <tr>
        <th>ID</th>
        <th>Name</th>
        <th>Tipo</th>
        <th>Marca</th>
        <th>Tamanho</th>
        <th>Sexo</th>

        <th width="280px">Action</th>
    </tr>
    @foreach ($artigos as $artigo)
    <tr>
        <td>{{ $artigo->id }}</td>
        <td>{{ $artigo->name }}</td>
        <td>{{ $artigo->tipo }}</td>
        <td>{{ $artigo->marca }}</td>
        <td>{{ $artigo->tamanho }}</td>
        <td>{{ $artigo->sexo }}</td>
        <td>
            <form action="{{ route('artigos.destroy',$artigo->id) }}" method="POST">

                <a class="btn btn-info" href="{{ route('artigo.show',$artigo->id) }}">Apresentar</a>

                <a class="btn btn-primary" href="{{ route('artigo.edit',$artigo->id) }}">Editar</a>

                @csrf
                @method('DELETE')

                <button type="submit" class="btn btn-danger">Eliminar</button>
            </form>
        </td>
    </tr>
    @endforeach
</table>


<!--
<table class="table table-bordered">
    <tr>
        <th>No</th>
        <th>Reference</th>
        <th>Brand</th>
        <th>Capacity</th>
        <th>Airport ID</th>
        <th width="280px">Action</th>
    </tr>
    @foreach ($flights as $flight)
    <tr>
        <td>{{ $flight->id }}</td>
        <td>{{ $flight->reference }}</td>
        <td>{{ $flight->brand }}</td>
        <td>{{ $flight->capacity }}</td>
        <td>{{ $flight->airport->name }}</td>
        <td>
            <form action="{{ route('flights.destroy',$flight->id) }}" method="POST">

                <a class="btn btn-info" href="{{ route('flights.show',$flight->id) }}">Show</a>
                @csrf
                @method('DELETE')

                <button type="submit" class="btn btn-danger">Delete</button>
            </form>
        </td>
    </tr>
    @endforeach
</table>
-->


@endsection